        ...
        <div className="flex-1 overflow-hidden px-8 sm:px-16 md:px-24 flex flex-col  mt-6">
          <div className="flex-1 flex flex-col justify-center max-w-md w-full mx-auto py-12">
            ...
            <AnimatePresence mode="wait">
              {(isFromGoogle || error) && (
                ...
                >
                  {isFromGoogle && (
                    <div className="p-3 rounded-xl bg-indigo-50 border border-indigo-200 text-indigo-700 text-[10px] font-semibold flex items-center gap-3 mb-3">
                      <div className="w-4 h-4 shrink-0 bg-indigo-100 rounded-full flex items-center justify-center">
                        <svg className="w-2.5 h-2.5" viewBox="0 0 24 24" fill="currentColor">
                          <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
                          <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
                        </svg>
                      </div>
                      <span className="leading-tight">Google info pre-filled — enter <strong>mobile</strong> & <strong>address</strong>.</span>
                    </div>
                  )}
                  {error && (
                    <div className="p-3 rounded-xl bg-red-50 border border-red-100 text-red-600 font-bold text-[11px] flex items-center gap-3">
          ...
          <form onSubmit={handleRegister} className="space-y-5">
            ...
            <motion.button
              ...
              type="submit"
              disabled={loading}
              className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-indigo-200 transition-all disabled:bg-indigo-400 flex items-center justify-center gap-3 text-xs uppercase tracking-[0.2em] mt-2"
            >
              {loading ? (
          ...
          <div className="mt-8">
            <div className="relative mb-6">
              ...
              <div className="relative flex justify-center text-[10px] uppercase tracking-widest">
                <span className="bg-white px-2 text-slate-400 font-bold tracking-tighter">Or register with social</span>
              </div>
            </div>
